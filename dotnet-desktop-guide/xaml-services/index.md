...
言語としての XAML に関する入門資料をお探しの場合は、[XAML の概要 (WPF)](../net/wpf/fundamentals/xaml.md)に関するページを参照してみてください。 そのトピックでは、Windows Presentation Foundation (WPF) にも、XAML マークアップおよび XAML 言語機能の使い方にも初心者である対象ユーザー向けに XAML が説明されています。 その他にも有用なドキュメントとして、[XAML 言語仕様](/previous-versions/msp-n-p/ff650760(v=pandp.10))内の入門資料があります。
