...
1. メソッドで、<xref:System.Windows.Forms.Form.LayoutMdi%2A> メソッドを使用して MDI 親フォームの <xref:System.Windows.Forms.MdiLayout> 列挙体を設定します。 次に、MDI 親フォーム (<xref:System.Windows.Forms.MdiLayout.Cascade?displayProperty=nameWithType>) の子ウィンドウに `Form1` 列挙値を使用する例を示します。 列挙体は、[ <xref:System.Windows.Forms.Control.Click> **重ねて Windows** ] メニュー項目のイベントのイベントハンドラー中にコード内で使用されます。  
