...
- イベント <xref:System.Windows.Forms.DataGrid.HitTest%2A> やイベントなど、適切なイベントハンドラーでメソッドを呼び出し <xref:System.Windows.Forms.Control.MouseDown> <xref:System.Windows.Forms.Control.Click> ます。  
  
     メソッドは、 <xref:System.Windows.Forms.DataGrid.HitTest%2A> <xref:System.Windows.Forms.DataGrid.HitTestInfo> クリックされた領域の行と列を含むオブジェクトを返します。  
