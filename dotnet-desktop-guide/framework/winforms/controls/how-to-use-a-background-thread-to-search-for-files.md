...
コンポーネントは、 <xref:System.ComponentModel.BackgroundWorker> 名前空間に対して機能を追加します。ただし、名前空間は <xref:System.Threading> <xref:System.Threading> 下位互換性と将来の使用の両方のために保持されます (選択した場合)。 詳細については、「 [BackgroundWorker コンポーネントの概要](backgroundworker-component-overview.md)」を参照してください。

 Windows フォームは、シングルスレッドアパートメント (STA) モデルを使用します。これは、Windows フォームが本質的にアパートメントスレッドであるネイティブの Win32 ウィンドウに基づいているためです。 STA モデルは、任意のスレッドでウィンドウを作成できることを意味しますが、作成されたスレッドを切り替えることはできません。また、スレッドに対するすべての関数呼び出しは、作成スレッドで実行する必要があります。 Windows フォーム外部では、.NET Framework のクラスはフリースレッドモデルを使用します。 .NET Framework のスレッド処理の詳細については、「 [スレッド処理](/dotnet/standard/threading/index)」を参照してください。

 STA モデルでは、コントロールの作成スレッドの外部から呼び出す必要があるコントロールのメソッドは、コントロールの作成スレッドにマーシャリングされる必要があります。 基底クラスには、 <xref:System.Windows.Forms.Control> この目的でいくつかのメソッド (、、および) が用意されて <xref:System.Windows.Forms.Control.Invoke%2A> <xref:System.Windows.Forms.Control.BeginInvoke%2A> <xref:System.Windows.Forms.Control.EndInvoke%2A> います。 <xref:System.Windows.Forms.Control.Invoke%2A> 同期メソッド呼び出しを行います。 <xref:System.Windows.Forms.Control.BeginInvoke%2A> 非同期のメソッド呼び出しを行います。

 リソースを集中的に使用するタスクのためにコントロールでマルチスレッドを使用する場合、リソースを集中的に使用する計算がバックグラウンドスレッドで実行されている間は、ユーザーインターフェイスの応答性を維持できます。

 次のサンプル () は、バックグラウンドスレッドを使用して、 `DirectorySearcher` 指定された検索文字列に一致するファイルをディレクトリで再帰的に検索し、検索結果をリストボックスに挿入する、マルチスレッド Windows フォームコントロールを示しています。 このサンプルで示す主な概念は次のとおりです。

- `DirectorySearcher` 新しいスレッドを開始して検索を実行します。 スレッドは、メソッドを実行して、 `ThreadProcedure` `RecurseDirectory` 実際の検索を実行し、リストボックスにデータを設定するためにヘルパーメソッドを呼び出します。 ただし、リストボックスを設定するには、次の2つの箇条書き項目で説明されているように、スレッド間の呼び出しが必要です。

- `DirectorySearcher` ファイルを `AddFiles` リストボックスに追加するメソッドを定義します。ただし、は、を `RecurseDirectory` `AddFiles` `AddFiles` 作成した STA スレッドでしか実行できないため、を直接呼び出すことはできません `DirectorySearcher` 。

- を `RecurseDirectory` 呼び出すことができるの `AddFiles` は、スレッド間の呼び出し (つまり、またはを呼び出して <xref:System.Windows.Forms.Control.Invoke%2A> <xref:System.Windows.Forms.Control.BeginInvoke%2A> `AddFiles` の作成スレッドにマーシャリングする) だけです `DirectorySearcher` 。 `RecurseDirectory` を使用し <xref:System.Windows.Forms.Control.BeginInvoke%2A> て、呼び出しを非同期的に実行できるようにします。

- メソッドをマーシャリングするには、関数ポインターまたはコールバックと同等のものが必要です。 これは、.NET Framework のデリゲートを使用して実現されます。 <xref:System.Windows.Forms.Control.BeginInvoke%2A> デリゲートを引数として受け取ります。 `DirectorySearcher` このため、デリゲート () を定義し `FileListDelegate` 、 `AddFiles` コンストラクター内ののインスタンスにバインド `FileListDelegate` し、このデリゲートインスタンスをに渡し <xref:System.Windows.Forms.Control.BeginInvoke%2A> ます。 `DirectorySearcher` また、検索の完了時にマーシャリングされるイベントデリゲートを定義します。
...
## <a name="using-the-multithreaded-control-on-a-form"></a>フォームでのマルチスレッドコントロールの使用
 ...
 次の例は、マルチスレッドコントロールをフォームで使用する方法を示して `DirectorySearcher` います。
