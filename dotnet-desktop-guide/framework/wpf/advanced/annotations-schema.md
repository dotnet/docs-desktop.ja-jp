...
ms.locfileid: "96984023"
---
# <a name="annotations-schema"></a><span data-ttu-id="fae45-102">注釈スキーマ</span><span class="sxs-lookup"><span data-stu-id="fae45-102">Annotations Schema</span></span>

<span data-ttu-id="fae45-103">このトピックでは、Microsoft Annotations Framework でユーザー注釈データの保存と取得に使用される XML スキーマ定義 (XSD) について説明します。</span><span class="sxs-lookup"><span data-stu-id="fae45-103">This topic describes the XML schema definition (XSD) used by the Microsoft Annotations Framework to save and retrieve user annotation data.</span></span>

<span data-ttu-id="fae45-104">Annotations Framework では、注釈データが内部表現から XML 形式にシリアル化されます。</span><span class="sxs-lookup"><span data-stu-id="fae45-104">The Annotations Framework serializes annotation data from an internal representation to an XML format.</span></span>  <span data-ttu-id="fae45-105">この変換に使用される XML 形式は Annotations Framework XSD スキーマで表現されます。</span><span class="sxs-lookup"><span data-stu-id="fae45-105">The XML format used for this conversion is described by the Annotations Framework XSD Schema.</span></span>  <span data-ttu-id="fae45-106">このスキーマは実装非依存の XML 形式を定義します。実装非依存の XML 形式を利用し、アプリケーション間で注釈データを交換できます。</span><span class="sxs-lookup"><span data-stu-id="fae45-106">The schema defines the implementation-independent XML format that can be used to exchange annotation data between applications.</span></span>

<span data-ttu-id="fae45-107">Annotations Framework XML スキーマ定義は 2 つのサブスキーマで構成されています。</span><span class="sxs-lookup"><span data-stu-id="fae45-107">The Annotations Framework XML schema definition consists of two subschemas</span></span>

- <span data-ttu-id="fae45-108">注釈 XML コア スキーマ (コア スキーマ)。</span><span class="sxs-lookup"><span data-stu-id="fae45-108">The Annotations XML Core Schema (Core Schema).</span></span>

- <span data-ttu-id="fae45-109">注釈 XML 基本スキーマ (基本スキーマ)。</span><span class="sxs-lookup"><span data-stu-id="fae45-109">The Annotations XML Base Schema (Base Schema).</span></span>

<span data-ttu-id="fae45-110">コア スキーマでは、<xref:System.Windows.Annotations.Annotation> のプライマリ XML 構造が定義されています。</span><span class="sxs-lookup"><span data-stu-id="fae45-110">The Core Schema defines the primary XML structure of an <xref:System.Windows.Annotations.Annotation>.</span></span>  <span data-ttu-id="fae45-111">コア スキーマに定義されている XML 要素の多くが <xref:System.Windows.Annotations> 名前空間の型に対応します。</span><span class="sxs-lookup"><span data-stu-id="fae45-111">The majority of XML elements defined in the Core Schema correspond to types in the <xref:System.Windows.Annotations> namespace.</span></span>  <span data-ttu-id="fae45-112">コア スキーマは 3 つの拡張ポイントを公開します。そのポイントでアプリケーションは独自の XML データを追加できます。</span><span class="sxs-lookup"><span data-stu-id="fae45-112">The Core Schema exposes three extension points where applications can add their own XML data.</span></span>  <span data-ttu-id="fae45-113">これらの拡張ポイントには、<xref:System.Windows.Annotations.Annotation.Authors%2A>、<xref:System.Windows.Annotations.ContentLocatorPart>、および "Content" が含まれます。</span><span class="sxs-lookup"><span data-stu-id="fae45-113">These extension points include the <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>, and "Content".</span></span>  <span data-ttu-id="fae45-114">(Content 要素は <xref:System.Xml.XmlElement> リストの形式で提供されます。)</span><span class="sxs-lookup"><span data-stu-id="fae45-114">(Content elements are provided in the form of an <xref:System.Xml.XmlElement> list.)</span></span>

<span data-ttu-id="fae45-115">このトピックで説明する基本スキーマでは、<xref:System.Windows.Annotations.Annotation.Authors%2A>、<xref:System.Windows.Annotations.ContentLocatorPart>、および初回の Windows Presentation Foundation (WPF) リリースに付属の Content 型の拡張が定義されています。</span><span class="sxs-lookup"><span data-stu-id="fae45-115">The Base Schema described in this topic defines the extensions for the <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>, and Content types included with the initial Windows Presentation Foundation (WPF) release.</span></span>

<a name="CoreSchema"></a>

## <a name="annotations-xml-core-schema"></a><span data-ttu-id="fae45-116">注釈 XML コア スキーマ</span><span class="sxs-lookup"><span data-stu-id="fae45-116">Annotations XML Core Schema</span></span>

<span data-ttu-id="fae45-117">注釈 XML コア スキーマでは、<xref:System.Windows.Annotations.Annotation> オブジェクトの保存に使用される XML 構造が定義されています。</span><span class="sxs-lookup"><span data-stu-id="fae45-117">The Annotations XML Core Schema defines the XML structure that is used to store <xref:System.Windows.Annotations.Annotation> objects.</span></span>
...
## <a name="annotations-xml-base-schema"></a><span data-ttu-id="fae45-118">注釈 XML 基本スキーマ</span><span class="sxs-lookup"><span data-stu-id="fae45-118">Annotations XML Base Schema</span></span>

<span data-ttu-id="fae45-119">基本スキーマでは、コア スキーマで定義された 3 つの抽象要素 (<xref:System.Windows.Annotations.Annotation.Authors%2A>、<xref:System.Windows.Annotations.ContentLocatorPart>、および <xref:System.Windows.Annotations.AnnotationResource.Contents%2A>) の XML 構造が定義されています。</span><span class="sxs-lookup"><span data-stu-id="fae45-119">The Base Schema defines the XML structure for the three abstract elements defined in the Core Schema – <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>, and <xref:System.Windows.Annotations.AnnotationResource.Contents%2A>.</span></span>
...
## <a name="sample-xml-produced-by-annotations-xmlstreamstore"></a><span data-ttu-id="fae45-120">注釈 XmlStreamStore で作成されたサンプル XML</span><span class="sxs-lookup"><span data-stu-id="fae45-120">Sample XML Produced by Annotations XmlStreamStore</span></span>

<span data-ttu-id="fae45-121">続く XML では、注釈 <xref:System.Windows.Annotations.Storage.XmlStreamStore> の出力と、3 つの注釈、すなわち、蛍光ペン、テキスト付箋、インク付箋を含むサンプル ファイルの編成が表示されます。</span><span class="sxs-lookup"><span data-stu-id="fae45-121">The XML that follows shows the output of an Annotations <xref:System.Windows.Annotations.Storage.XmlStreamStore> and the organization of a sample file that contains three annotations - a highlight, a text sticky-note, and an ink stick-note.</span></span>
...
## <a name="see-also"></a><span data-ttu-id="fae45-122">関連項目</span><span class="sxs-lookup"><span data-stu-id="fae45-122">See also</span></span>
...
- [<span data-ttu-id="fae45-123">注釈の概要</span><span class="sxs-lookup"><span data-stu-id="fae45-123">Annotations Overview</span></span>](annotations-overview.md)
